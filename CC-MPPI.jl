...
R = Matrix{Float64}(I, u_dim, u_dim)
...
function main()
    ...
    Um_mppi_list = zeros(Float64, M, N, u_dim)
    ...
    # anim = @animate while abs(X_ref[1,1]-goal[1]) > 1
        ...
        #Execute Command
        X_ref_old = copy(X_ref) # for plotting
        ...
        for m in 1:M
            Xm_mppi = copy(X_ref_old)
            for k in 1:N
                Xm_mppi[k+1,:] = BicycleModelDynamics(Xm_mppi[k,:], Um_mppi_list[m,k,:])'
            end
            plot!(Xm_mppi[:,1], Xm_mppi[:,2], line = (1, :lightskyblue), legend = false, ylims=(-5,5), xlims=(-10,10))
        end
        for m in 1:M
            X_m = copy(X_ref_old)
            for k in 1:N
